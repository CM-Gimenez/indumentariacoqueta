const Otonioinvierno = require('../models/otonioinvierno.model.js')
const fs = require('fs')
...
exports.create = async (options) => {
  ...
  const SingleUntitled = new Otonioinvierno(updatedData)
...
// Retrieve and return all Otonioinvierno from the database.
exports.findAll = (options) => {
  ...
  Otonioinvierno.find(findString)
    .sort(query.sort && { [query.sort.field]: query.sort.method === 'desc' ? -1 : 1 })

    .then((otonioinvierno) => {
      options.res.json(paginate.paginate(otonioinvierno, { page: query.page, limit: query.limit || 10 }))
    })
    .catch((err) => {
      options.res.status(500).send({
...
exports.find = (options) => {
  return new Promise((resolve, reject) => {
    ...
    if (query.searchField) {
      if (Otonioinvierno.schema.path(query.searchField).instance === 'Boolean') {
        findString = { [query.searchField]: JSON.parse(query.searchString) }
      } else if (Otonioinvierno.schema.path(query.searchField).instance === 'Date') {
      ...
      if (
        Otonioinvierno.schema.path(query.searchField).instance === 'ObjectID' ||
        Otonioinvierno.schema.path(query.searchField).instance === 'Array'
      ) {
...
exports.update = (options) => {
  return new Promise(async (resolve, reject) => {
    ...
    const query = { populate: 'true' }
    Otonioinvierno.findByIdAndUpdate(id, updatedData, { new: true })
...
exports.delete = (options) => {
  return new Promise((resolve, reject) => {
    ...
    Otonioinvierno.deleteMany(theFilter)
      .then((result) => {
        resolve(result)
      })
      .catch((e) => {

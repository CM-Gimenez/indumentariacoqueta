...
exports.create = async (options) => {
  ...
  if (options.req.files && options.req.files.image && options.req.files.image.data) {
  ...
  const SingleUntitled = new Producto(updatedData)
...
// Retrieve and return all Producto from the database.
exports.findAll = (options) => {
  ...
  Producto.find(findString)
    .sort(query.sort && { [query.sort.field]: query.sort.method === 'desc' ? -1 : 1 })

    .then((producto) => {
      options.res.json(paginate.paginate(producto, { page: query.page, limit: query.limit || 10 }))
    })
    .catch((err) => {
      options.res.status(500).send({
...
exports.find = (options) => {
  return new Promise((resolve, reject) => {
    ...
    if (query.searchField) {
      if (Producto.schema.path(query.searchField).instance === 'Boolean') {
        findString = { [query.searchField]: JSON.parse(query.searchString) }
      } else if (Producto.schema.path(query.searchField).instance === 'Date') {
        findString = { $expr: { $eq: [query.searchString, { $dateToString: { date: `$${query.searchField}`, format: '%Y-%m-%d' } }] } }
      } else {
        findString = { [query.searchField]: { $regex: new RegExp(query.searchString, 'i') } }
      }

      if (Producto.schema.path(query.searchField).instance === 'ObjectID' || Producto.schema.path(query.searchField).instance === 'Array') {
...
exports.update = (options) => {
  return new Promise(async (resolve, reject) => {
    ...
    const query = { populate: 'true' }
    Producto.findByIdAndUpdate(id, updatedData, { new: true })
